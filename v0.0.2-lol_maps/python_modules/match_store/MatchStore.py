...
class MatchStore:
    ...
    def store_match(self, match: Match) -> None:
        """Store with expiry-aware tiering"""
        expiry_date = datetime.fromtimestamp(match.expires_at / 1000)
        days_until_expiry = (expiry_date - datetime.now()).days
        
        stored = StoredMatch(
            **match.model_dump(),  # Changed
            id=match.match_id,
            last_retrieved=int(datetime.now().timestamp() * 1000),
            access_count=0
        )
        
        if days_until_expiry > 7:
            self.hot_db.insert_one(stored.model_dump())  # Changed
        else:
            self.cold_db.insert_one(self._compress_match(stored.model_dump()))  # Changed
    
    def retrieve_match(self, match_id: str, participant_puuid: Optional[str] = None) -> Optional[Match]:
        """Retrieve from hot or cold storage"""
        doc = self.hot_db.find_one({"_id": match_id})
        if not doc:
            doc = self.cold_db.find_one({"_id": match_id})
            if doc:
                doc = self._decompress_match(doc)
        
        if doc and participant_puuid:
            doc['participants'] = [p for p in doc['participants'] if p['puuid'] == participant_puuid]
        
        return Match(**doc) if doc else None
    
    @staticmethod
    def _compress_match(match: dict) -> dict:
        return {
            k: v for k, v in match.items() 
            if k in {'_id', 'match_id', 'teams', 'participants', 'key_moments', 'win_conditions', 'expires_at', 'game_duration'}
        }
    
    @staticmethod
    def _decompress_match(compressed: dict) -> dict:
        return compressed
